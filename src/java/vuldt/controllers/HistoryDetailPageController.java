 ...
 */
public class HistoryDetailPageController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String quizID = "", email = "", subjectID = "";
            if (request.getParameter("txtQuizID") != null) {
                quizID = (String) request.getParameter("txtQuizID");
            }
            ...
            list = daoTakeQuiz.getListQuizDetailHistory(email, subjectID, quizID, 1);
            session.setAttribute("LIST_DETAIL_TAKE_QUIZ", list);
            int numberQuestion = 0, pageSize = 10, numberOnePage = 0;
            numberQuestion = daoTakeQuiz.getNumTakeQuizHistory(email, subjectID, quizID);
            numberOnePage = numberQuestion / pageSize;
            if (numberQuestion % pageSize != 0) {
                numberOnePage++;
            }
            session.setAttribute("NUMBER_PAGE_DETAIL", numberOnePage);
            ...
            listAnswer = daoAnswer.getAnswerByQuizID(quizID);
            session.setAttribute("LIST_ANSWER_QUIZ", listAnswer);
        } catch (Exception e) {
            log("Error in HistoryDetailPageController: " + e.getMessage());
        } finally {
