...
public class UpdatePageController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            question.setSubjectID(daoSubject.getSubjectName(question.getSubjectID()));
            ArrayList<AnswerDTO> listAnswer = new ArrayList<>();
            listAnswer = daoAnswer.getAnswerByID(questionID);
            session.setAttribute("QUESTION_UPDATE", question);
            session.setAttribute("LIST_ANSWER_UPDATE", listAnswer);
        } catch (Exception e) {
            log("Error in UpdatePageController: " + e.getMessage());
        } finally {
